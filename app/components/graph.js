...
export default class GraphComponent extends Component {
  ...
  get timestamps(){
    return this.postStream.getPostsByType(this.args.type).map(post => post.timestamp)
  ...
  drawGraph = modifier((element) => {
    ...
    const xScale = d3.scaleBand().domain(d3.range(24))
      .range([margin.left, width - margin.right]).padding(0.1);
  
    // Y = Jour (Dimanche → Samedi)
    const yScale = d3.scaleBand().domain([0, 1, 2, 3, 4, 5, 6])
    ...
    svg.selectAll("circle")
      ...
      .style("fill", d => counts[d] === maxPosts && maxPosts !==1 ? "red" : "black") // 📌 Le plus grand en rouge
      .style("opacity", 0.7)
